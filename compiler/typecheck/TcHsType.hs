...
tcHsDeriv hs_ty
  = do { ty <- checkNoErrs $  -- Avoid redundant error report
       ...
       ; pred_kind <- tcTypeKind pred
       ; let (tvs, pred)    = splitForAllTys ty
             (kind_args, _) = splitFunTys pred_kind
...
tc_hs_tuple_type mode rn_ty HsBoxedOrConstraintTuple hs_tys exp_kind
  ...
  = do { traceTc "tc_hs_type tuple 2" (ppr hs_tys)
       ...
       ; kinds <- mapM tcTypeKind tys -- Expose as much kind information as poss
...
tc_tuple :: HsType GhcRn -> TcTyMode
         -> TupleSort -> [LHsType GhcRn]
         -> TcKind -> TcM TcType
tc_tuple rn_ty mode tup_sort tys exp_kind
...
finish_tuple rn_ty tup_sort tau_tys tau_kinds exp_kind
  = do { traceTc "finish_tuple" (ppr tau_kinds $$ ppr exp_kind)
...
checkExpectedKindX pp_hs_ty ty exp_kind
 = do { -- We need to make sure that both kinds have the same number of implicit
   ...
   do { let origin = TypeEqOrigin { uo_actual   = act_kind'
      ...
      ; traceTc "checkExpectedKind: adding cast" $
        vcat [ ppr ty' <+> dcolon <+> ppr act_kind'
             , ppr exp_kind
             , ppr co_k ]
...
tcTyVar mode name         -- Could be a tyvar, a tycon, or a datacon
  = do { traceTc "lk1" (ppr name)
       ...
       ; case thing of
           ...
           AGlobal (ATyCon tc)
             -> do { traceTc "tcTyVar1b" (ppr tc <+> ppr (tyConKind tc))
                   ; check_tc tc
                   ; handle_tyfams tc }
...
the arguments again to give good error messages in
    eg. `(Maybe, Maybe)`
